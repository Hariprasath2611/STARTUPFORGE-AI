  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold font-display">Co-Founder Matching Portal</h1>
          <p className="text-xs text-textSecondary mt-1">Discover builders, technical leads, and growth marketers. Order by AI Compatibility Score.</p>
        </div>
        <button 
      ...
      {loading ? (
        <div className="glass-panel p-12 text-center text-xs text-textSecondary flex items-center justify-center space-x-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div key={idx} className="glass-panel p-6 rounded-2xl flex flex-col justify-between space-y-4 border border-borderBg hover:border-primary/20 transition-all relative overflow-hidden">
              ...
              <div className="space-y-3">
                <div className="flex items-center space-x-3">
                  <div className="h-10 w-10 rounded-full bg-primary/20 flex items-center justify-center text-primary font-bold uppercase text-sm">
                    {cand.user.name[0]}
                  </div>
                  <div>
                    <h4 className="text-sm font-bold text-white">{cand.user.name}</h4>
                    <span className="text-[9px] px-2 py-0.5 rounded bg-surface border border-borderBg uppercase font-bold text-primary tracking-wider inline-flex items-center">
                ...
                <p className="text-xs text-textSecondary leading-relaxed">{cand.user.profileWizard?.bio}</p>

                <div className="flex items-center space-x-1.5 text-[10px] text-textSecondary">
                ...
                <div className="space-y-1.5">
                  <span className="text-[9px] font-bold uppercase text-textSecondary tracking-wider block">Skills & Toolsets</span>
                  <div className="flex flex-wrap gap-1.5">
                    {cand.user.profileWizard?.skills.map((s: string, i: number) => (
                      <span key={i} className="px-2 py-0.5 rounded bg-surface border border-borderBg text-[9px] text-textSecondary">{s}</span>
                    ))}
                  </div>
                </div>
                ...
                <div className="p-3 bg-surface rounded-xl border border-borderBg text-[10px] text-textSecondary">
                  <span className="font-bold text-white block mb-0.5">AI Heuristics Analysis:</span>
                  {cand.analysis}
                </div>
              </div>

              {/* Actions */}
              <div className="pt-3 border-t border-borderBg/50">
                <button
                  onClick={() => handleRequestCollaboration(cand.user.name)}
                  className="w-full py-2 bg-primary hover:bg-secondary text-black font-semibold text-xs rounded-lg shadow-glow flex items-center justify-center space-x-1.5 transition-all"
                >
          ...
          {candidates.length === 0 && (
            <div className="glass-panel p-12 text-center text-textSecondary text-xs md:col-span-2">
